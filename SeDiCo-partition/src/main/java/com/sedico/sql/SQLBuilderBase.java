...
import java.util.*;
/**
 * Dises Klasse lässt Tabellen erzeugen, Inhalte in Tabellen einfügen und Tabellen aktualisieren. Die Klasse implementiert das Interface SQLBuilder.
 * @author jens
 *
 */
public abstract class SQLBuilderBase implements SQLBuilder {
	 ...
	 * @param table - Tabelle, die erzeugt werden soll
    ...
    public String buildCreateTable(Table table, String databaseName) {
        ...
        sql.append(String.format("CREATE TABLE %s.%s (", databaseName, table.getTableName()));
        //sql.append("CREATE TABLE " + databaseName + "." + table.getTableName() + "(");
        for (ColumnDescriptor column : table.getPrimaryColumnDescriptors()) {
            ...
            switch (dataType) {
            	case "varchar(0)": sql.append("varchar(255)");
            		break;
            	case "decimal(0,-127)":
            	case "decimal(0,0)": sql.append("int");
            		break;
            
            	default: sql.append(column.getDataTypeForDialect(getDialect()));
            }
        ...
        for (ColumnDescriptor column : table.getValueColumnDescriptors()) {
            ...
            switch (dataType) {
            	...
            	case "varchar(0)": dataType = "varchar(255)";
            		sql.append(dataType);
            		break;
            	case "varchar2(0 char)": dataType = "varchar2(255)";
            		sql.append(dataType);
            		break;
            	case "decimal(0,-127)":
            	case "decimal(0,0)": dataType = "int";
            		sql.append(dataType);
            		break;
            	default: sql.append(column.getDataTypeForDialect(getDialect()));
            }
     ...
     * @param table - Tabelle, in die Inhalte eingefügt werden
     * @param databaseName - Name der Datenbank
            	...
            	*/
            	if (column.getColumnValue() != null) {
            		switch (dataTypeColumn) {
	            		case 91:
	            		case 93:
	            		case 12: 	sql.append("'" + column.getColumnValue() + "'");
	            					break;
	            		case 16: 	if (column.getColumnValue().equals("true")) {sql.append("1"); } else { sql.append("0"); }
	            					break;
	            		case 1: 	if (column.getColumnValue().equals("Y")) {sql.append("1"); } else { sql.append("0"); }
									break;
	            		case -7:
	            		case 3:
	            		case 4: 	sql.append(column.getColumnValue());
	            					break;
     ...
     * @param table - Tabelle, in der Inhalte gelöscht werden
     * @param databaseName - Name der Datenbank
    ...
    public List<String> buildDeletes(Table table, String databaseName) {
        ...
        StringBuilder sql = new StringBuilder();
        for (Row row : table.getRows()) {
            sql.append(String.format("DELETE FROM %s.%s", databaseName, table.getTableName()));
     ...
     * @param table - Tabelle, welche aktualisiert werden soll
     * @param databaseName - Name der Datenbank
    ...
    public List<String> buildUpdates(Table table, String databaseName) {
        ...
        StringBuilder sql = new StringBuilder();
        for (Row row : table.getRows()) {
            sql.append(String.format("UPDATE %s.%s SET ", databaseName, table.getTableName()));
            for (ColumnDescriptor descriptor : table.getValueColumnDescriptors()) {
                ...
                //Eine Spalte kann einen Descriptor aber keine Daten haben, wenn die Daten in der anderen Partition sind.
                if (column != null) {
                    ...
                    CharSequence seq2 = "/";
                 	...
                 	switch (dataTypeColumn) {
                 		...
                 		case 91:
                 		case 93:
                 		case 12: 	if (column.getColumnValue() != null) {sql.append("'" + column.getColumnValue() + "'");} else {sql.append("''");};
                 					break;
                 		case 16: 	if (column.getColumnValue().equals("true")) {sql.append("1"); } else { sql.append("0"); }
                 					break;
                 		case -7:
                 		case 3: 
                 		case 4: 	if (column.getColumnValue() != null) {sql.append("'" + column.getColumnValue() + "'");} else {sql.append("''");};
                 					
                 					break;
                 		case 2000:	
                 			if (column.getColumnValue() != null) {
                 				if (column.getColumnValue().toString().contains(seq1) || column.getColumnValue().toString().contains(seq2)) {
                 					sql.append("'" + column.getColumnValue() + "'");
                 				} 
                 				else {
                 					sql.append(column.getColumnValue());
    ...
    /**
     * Die Methode gibt die Namen der Tabelle, Spalten und Datenbank als String zurück
     * @param Tabelle - Tabelle, die durchlaufen wird
     * @param databaseName - Name der Datenbank
     * @return sql.toString() - Die Informationen werden als String zurückgegeben
     */
    @Override
    ...
    /**
     * Die Methode bildet einen String von allen Spalten und ihren Werten mit jedem Spaltenbeschreiber.
     * @param sql - StringBuilder
     * @param table - Tabelle
     * @param row - Zeile in der Tabelle
     */
    private void addWhereClauseForPrimaryKeys(StringBuilder sql, Table table, Row row) {
        sql.append(" WHERE ");
        for(ColumnDescriptor descriptor : table.getPrimaryColumnDescriptors()) {
            String columnName = descriptor.getColumnName();
            Column column = row.getColumn(columnName);
            if (column == null) {
                throw new NoSuchElementException("Es wurde eine Spalte " + columnName + " angefordert, die nicht existiert oder nicht als PK definiert ist.");
            }
            sql.append(columnName);
            sql.append(" = ");
            sql.append("'" + column.getColumnValue() + "'");
